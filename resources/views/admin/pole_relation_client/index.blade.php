...
@section('title', 'Gestion des pôles relation clients')
@section('content')
    <div class="container-fluid">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0">
                                ...
                                <li class="breadcrumb-item active text-dark fw-semibold" aria-current="page">
                                    Pôles relation clients
                                </li>
                            </ol>
                        </nav>
                    </div>
                    <div class="btn-group">
                        <a href="{{ route('download.prc.model') }}" class="btn btn-outline-success">
                            <i class="bx bx-download me-1"></i> Modèle PRC
                        </a>
                        ...
                        <a href="{{ route('pole_relation_clients.create') }}" class="btn btn-primary ms-2">
                            <i class="bx bx-plus me-1"></i> Nouveau PRC
                        </a>
                    </div>
                </div>
            </div>
        ...
        @if (session('ignoredEmails'))
            <div class="alert alert-warning border-0 alert-dismissible fade show shadow-sm mb-4">
                <div class="d-flex align-items-center">
                    ...
                    <span class="fw-medium">Pôles relation clients déjà existants</span>
                </div>
                <div class="mt-2">
                ...
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        <h5 class="modal-title text-white" id="importModalLabel">
                            <i class="lni lni-cloud-download me-2"></i>Importer des pôles relation clients
                        </h5>
        ...
        <!-- Tableau des pôles relation clients -->
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-bottom-0 py-3">
                <h6 class="mb-0 text-dark fw-semibold">
                    <i class="bx bx-list-ul me-2"></i>Liste des pôles relation clients
                    <span class="badge bg-info text-dark ms-2">{{ $poleRelationClients->count() }}</span>
                </h6>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="prcTable" class="table table-hover table-striped align-middle mb-0">
                        ...
                        <tbody>
                            @foreach ($poleRelationClients as $prc)
                                <tr>
                                    <td>
                                        <span class="fw-semibold text-dark">{{ $prc->id }}</span>
                                    </td>
                                    <td>
                                        <div class="fw-semibold text-dark">{{ $prc->user->name }}</div>
                                    </td>
                                    <td>
                                        <div class="text-dark">{{ $prc->prenom }}</div>
                                    </td>
                                    <td>
                                        <a href="mailto:{{ $prc->user->email }}" class="text-decoration-none text-dark">
                                            {{ $prc->user->email }}
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('pole_relation_clients.show', $prc->id) }}"
                                                class="btn btn-info text-white">
                                                Afficher
                                            </a>
                                            <a href="{{ route('pole_relation_clients.edit', $prc->id) }}"
...
@section('scripts')
    <script>
        $(document).ready(function() {
            var hasData = {{ $poleRelationClients->count() > 0 ? 'true' : 'false' }};

            var table = $('#prcTable').DataTable({
                language: {
                    url: "https://cdn.datatables.net/plug-ins/1.13.5/i18n/fr-FR.json",
                    emptyTable: "Aucun pôle relation client trouvé."
                },
                paging: true,
                searching: true,
                ordering: true,
