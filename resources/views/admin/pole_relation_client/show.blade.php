...
@section('content')
   <div class="container">
       ...
       <div class="page-breadcrumb d-none d-sm-flex align-items-center">
           ...
           <div class="ps-3">
               <nav aria-label="breadcrumb">
                   <ol class="breadcrumb mb-0 p-0">
                       <li class="breadcrumb-item"><a href="{{ route('pole_relation_clients.index') }}"><i class="bx bx-home-alt"></i></a>
                       </li>
           ...
           <div class="ms-auto">
               <div class="btn-group">
                   <a href="{{ route('pole_relation_clients.index') }}" type="button" class="btn btn-sm btn-primary px-4"> <i
                           ...
                           <div class="row">
                               ...
                               <div class="text-center mb-4">
                                   ...
                                   <img src="{{ $poleRelationClient->user->image ? asset($poleRelationClient->user->image) : 'https://ui-avatars.com/api/?name=' . urlencode($poleRelationClient->user->name) . '&background=0D8ABC&color=fff&size=128' }}"
                                        class="rounded-circle shadow" width="200" height="200" alt="Avatar" style="object-fit: cover">
                                   <h3 class="mt-3 mb-1">{{ $poleRelationClient->user->name }}</h3>
                                   <span class="badge bg-info text-dark px-3 py-1">{{ ucfirst($poleRelationClient->user->role) }}</span>
                               </div>

                               <h2 class="text-center">@if($poleRelationClient->stagiaire){{$poleRelationClient->stagiaire->civilite}}@endif
                                   . {{$poleRelationClient->user->name}} @if($poleRelationClient->stagiaire){{$poleRelationClient->stagiaire->prenom}}@endif</h2>
                               ...
                               <div class="row mb-3">
                                   ...
                                   <div class="col-sm-8">
                                       <a href="mailto:{{ $poleRelationClient->user->email }}">{{ $poleRelationClient->user->email }}</a>
                                   </div>
                               </div>
                               ...
                               <div class="row">
                                   <div class="col-sm-12">
                                       ...
                                       <div class="card-body">
                                               ...
                                               @foreach ($poleRelationClient->stagiaires as $key => $row)
                                                   <div class="accordion-item mb-3">
                                                       <h2 class="accordion-header" id="heading{{ $key }}">
                                                           <button class="accordion-button d-flex justify-content-between align-items-center shadow-sm p-3"
                                                               ...
                                                               <div class="d-flex align-items-center">
                                                                   ...
                                                                   <div class="fw-bold">{{ $row->user->name }} {{ $row->prenom }}</div>
                                                               </div>
                                                           </button>
                                                       </h2>
                                                       <div id="collapse{{ $key }}" class="accordion-collapse collapse @if($key == 0) show @endif"
                                                            aria-labelledby="heading{{ $key }}" data-bs-parent="#stagiairesAccordion"  style="box-shadow: rgba(17, 12, 46, 0.15) 0px 48px 100px 0px;">
                                                           <div class="accordion-body p-4 bg-light shadow-sm rounded-3">
                                                               <div class="row">
                                                                   <div class="col-12 col-md-6 mb-3">
                                                                       <strong>Adresse:</strong> <span class="text-muted">{{ $row->adresse }}</span>
                                                                   </div>
                                                                   <div class="col-12 col-md-6 mb-3">
                                                                       <strong>Téléphone:</strong> <span class="text-muted">{{ $row->telephone }}</span>
                                                                   </div>
                                                                   <div class="col-12 col-md-6 mb-3">
                                                                       <strong>Date de naissance:</strong> <span class="text-muted">{{ $row->date_naissance }}</span>
                                                                   </div>
                                                                   <div class="col-12 col-md-6 mb-3">
                                                                       <strong>Ville:</strong> <span class="text-muted">{{ $row->ville }}</span>
                                                                   </div>
                                                                   <div class="col-12 col-md-6 mb-3">
                                                                       <strong>Code postal:</strong> <span class="text-muted">{{ $row->code_postal }}</span>
                                                                   </div>
                                                               </div>
                                           ...
                                           <form action="{{ route('pole_relation_clients.destroy', $poleRelationClient->id) }}" method="POST"
                                                 class="d-inline"
