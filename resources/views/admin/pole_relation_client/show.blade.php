...
@section('title', 'Détails du pôle relation client')
@section('content')
                ...
                <div
                    ...
                    <div class="mb-3 mb-md-0">
                        <h5 class="card-title mb-1 text-primary">
                            <i class="bx bx-user-circle me-2"></i>Détails du pôle relation client
                        </h5>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('pole_relation_clients.index') }}" class="text-decoration-none">
                                        <i class="bx bx-home-alt"></i> Pôles relation clients
                                    </a>
                                </li>
                                <li class="breadcrumb-item active text-dark fw-semibold" aria-current="page">
                                    {{ $poleRelationClient->user->name }} {{ $poleRelationClient->prenom }}
                                </li>
                            </ol>
                        </nav>
                    </div>
                    <div class="btn-group">
                        ...
                        <a href="{{ route('pole_relation_clients.index') }}" class="btn btn-outline-primary ms-2">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <div class="text-center mb-4">
                            ...
                            @if ($poleRelationClient->user->image)
                                <img src="{{ asset($poleRelationClient->user->image) }}" class="rounded-circle shadow"
                            ...
                            @else
                                <div class="bg-secondary text-white rounded-circle d-flex align-items-center justify-content-center mx-auto"
                                    ...
                                    {{ strtoupper(substr($poleRelationClient->user->name, 0, 1)) }}{{ strtoupper(substr($poleRelationClient->prenom, 0, 1)) }}
                                </div>
                            @endif
                            <h3 class="mt-3 mb-2 text-dark fw-bold">{{ $poleRelationClient->user->name }}
                                {{ $poleRelationClient->prenom }}</h3>
                            <span class="badge bg-primary px-3 py-2 fs-6">{{ ucfirst($poleRelationClient->role) }}</span>
                        </div>
                            ...
                            <div class="col-md-8">
                                <div class="card border-0 bg-light">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-sm-4 fw-semibold text-dark">Rôle :</div>
                                            <div class="col-sm-8">
                                                <span
                                                    class="badge bg-info text-dark">{{ ucfirst($poleRelationClient->role) }}</span>
                                            </div>
                                        </div>
                                        <div class="row mb-3">
                                            <div class="col-sm-4 fw-semibold text-dark">Email :</div>
                                            <div class="col-sm-8">
                                                <a href="mailto:{{ $poleRelationClient->user->email }}"
                                                    class="text-decoration-none text-dark">
                                                    <i
                                                        class="bx bx-envelope me-1"></i>{{ $poleRelationClient->user->email }}
                                                </a>
                                            </div>
                                        </div>
                                        @if ($poleRelationClient->telephone)
                                            <div class="row mb-3">
                                                <div class="col-sm-4 fw-semibold text-dark">Téléphone :</div>
                                                <div class="col-sm-8">
                                                    <a href="tel:{{ $poleRelationClient->telephone }}"
                                                        class="text-decoration-none text-dark">
                                                        <i
                                                            class="bx bx-phone me-1"></i>{{ $poleRelationClient->telephone }}
                                                    </a>
                                                </div>
                                            </div>
                                        @endif
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-transparent border-bottom-0 py-3">
                        <h6 class="mb-0 text-dark fw-semibold">
                            ...
                            <span
                                class="badge bg-info text-dark ms-2">{{ $poleRelationClient->stagiaires->count() }}</span>
                        </h6>
                    </div>
                    <div class="card-body">
                        @if ($poleRelationClient->stagiaires->count() > 0)
                            <div class="accordion" id="stagiairesAccordion">
                                @foreach ($poleRelationClient->stagiaires as $key => $stagiaire)
                        ...
                        @else
                            <div class="text-center py-4">
                                ...
                                <p class="text-muted mb-0">Aucun stagiaire associé à ce pôle relation client</p>
                                <a href="{{ route('pole_relation_clients.edit', $poleRelationClient->id) }}"
                ...
                <div class="card border-0 shadow-sm mt-4">
                    <div class="card-body text-center">
                        <div class="">
                            ...
                            <form action="{{ route('pole_relation_clients.destroy', $poleRelationClient->id) }}"
                                method="POST" class="d-inline ms-2"
                                onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce pôle relation client ?');">
                                @csrf
                                @method('DELETE')
