                ...
                <div class="card shadow-lg border-0">
                    <div class="card-body p-5">
                        <div class="text-center mb-4">
                            <form action="{{ route('parametre.updateImage', $user->id) }}" method="POST"
                                ...
                                <label for="imageInput" style="cursor: pointer; position: relative; display: inline-block;">
                                    <img src="{{ $user->image ? asset($user->image) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=0D8ABC&color=fff&size=128' }}"
                        ...
                        <div class="text-end mt-4">
                            <a href="{{ route('parametre.edit', $user->id) }}" class="btn btn-outline-warning btn-sm me-2">
                            ...
                            <form action="{{ route('parametre.destroy', $user->id) }}" method="POST" class="d-inline"
                                ...
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-outline-danger btn-sm"><i class="bx bx-trash"></i> Supprimer</button>
                            </form>
                        </div>
