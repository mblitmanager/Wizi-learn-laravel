               ...
               <div class="ps-3">
                   <nav aria-label="breadcrumb">
                       <ol class="breadcrumb mb-0 p-0">
                           ...
                           <li class="breadcrumb-item active"><a href="{{ route('catalogue_formation.index') }}">Détails d'un catalogue de formations.</a>
                           </li>
       ...
       <div class="col-md-12">
           <div class="card shadow-lg rounded">
               <div class="card-header bg-gradient-primary text-white">
                   <h5 class="mb-0"><i class="bx bx-book"></i> Détails de la Formation : {{ $catalogueFormations->titre }}</h5>
               </div>
               <div class="card-body">
                   <div class="row mb-4">
                       <div class="col-md-6">
                       ...
                       <div class="col-md-6">
                           ...
                           @if ($catalogueFormations->image_url)
                               @php
                                   $extension = strtolower(pathinfo($catalogueFormations->image_url, PATHINFO_EXTENSION));
                               @endphp

                               <div class="d-flex justify-content-center mt-4">
                                   @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'webp']))
                                       <img src="{{ asset('storage/' . $catalogueFormations->image_url) }}"
                                            alt="Image de la formation" class="img-fluid rounded shadow-lg" style="max-height: 250px;">
                                   @elseif (in_array($extension, ['mp4', 'webm', 'ogg']))
                                       <video controls class="rounded shadow-lg" style="max-height: 250px; width: auto;">
                                           <source src="{{ asset('storage/' . $catalogueFormations->image_url) }}" type="video/{{ $extension }}">
                                           Votre navigateur ne supporte pas la lecture de vidéos.
                                       </video>
                                   @elseif (in_array($extension, ['mp3', 'wav', 'ogg']))
                                       <div class="card p-3 rounded shadow-lg" style="max-width: 400px; background: linear-gradient(135deg, #fbdfa1, #ffb923); ">
                                           <div class="mb-2 text-center">
                                               <i class="bi bi-music-note-beamed" style="font-size: 2rem; color: #4a4a4a;"></i>
                                           </div>
                                           <audio controls>
                                               <source src="{{ asset('storage/' . $catalogueFormations->image_url) }}" type="audio/{{ $extension }}">
                                               Votre navigateur ne supporte pas la lecture d'audio.
                                           </audio>
                                       </div>
                                   @else
                                       <a href="{{ asset('storage/' . $catalogueFormations->image_url) }}" target="_blank" class="btn btn-outline-primary">
